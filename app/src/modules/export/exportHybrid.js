...
export async function exportAllVisibleLayers(map, layers) {
  ...
  // Single export frame shared by basemap PNG and all SVGs.
  const frame = getExportFrame(map)
  const { bounds, width, height } = frame
  if (!width || !height) throw new Error('Map container has invalid size')

  // Basemap PNG (tiles only).
  const mapContainer = map.getContainer()
  const containerRect = mapContainer.getBoundingClientRect()
  await waitForTiles(mapContainer)
  await waitNextFrame()
  ...
  // One SVG per visible compatible layer, sequential with small delays.
