...
function drawNorthArrow(ctx, canvasWidth) {
  const margin = 12
  const pad = 6
  const fontSize = 10
...
function drawScaleBar(ctx, map, canvasHeight) {
  const MAX_BAR_W = 120
...
export default function useMapExport() {
  const exportMapAsPNG = useCallback(async ({ map, fileName = 'editor-mapes.png', legendLayers = [], title = '', showLegend = true }) => {
